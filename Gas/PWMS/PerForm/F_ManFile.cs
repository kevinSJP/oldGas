...
//using Microsoft.Office 11.0 Object Library; 
...
namespace PWMS.PerForm
{
    public partial class F_ManFile : Form
    {
        ...
        private void Sut_Save_Click(object sender, EventArgs e)
        {
            if (tabControl1.SelectedTab.Name == "tabPage6") //如果当前是“个人简历”选项卡
            {
                ...
                if (Read_Memo.Read())   //如果有记录
                    //将当前设置的个人简历进行修改
                    DataClass.MyMeans.getsqlcom("update tb_Individual set Memo='" + Ind_Mome.Text + "' where ID='" + tem_ID + "'");
                else
                    //如果没有记录，则进行添加操作
                    DataClass.MyMeans.getsqlcom("insert into tb_Individual (ID,Memo) values('" + tem_ID + "','" + Ind_Mome.Text + "')");
            }
                ...
                try
                {
                    if (hold_n == 1 || hold_n == 2) //判断当前是添加，还是修改操作
                    {
                        ...
                        if (ModuleClass.MyModule.ADDs != "")
                            DataClass.MyMeans.getsqlcom(ModuleClass.MyModule.ADDs);
                    }
        ...
        private void Part_Delete_Click(object sender, EventArgs e)
        {
            ...
            {
                DataClass.MyMeans.getsqlcom("Delete " + Delete_Table + " where ID='" + Delete_ID + "'");
                Part_Cancel_Click(sender, e);
            }
        }
        ...
        private void Sut_Delete_Click(object sender, EventArgs e)
        {
            ...
            DataClass.MyMeans.getsqlcom("Delete tb_Stuffbusic where ID='" + S_0.Text.Trim() + "'");
            DataClass.MyMeans.getsqlcom("Delete tb_WordResume where Sut_ID='" + S_0.Text.Trim() + "'");
            DataClass.MyMeans.getsqlcom("Delete tb_Family where Sut_ID='" + S_0.Text.Trim() + "'");
            DataClass.MyMeans.getsqlcom("Delete tb_TrainNote where Sut_ID='" + S_0.Text.Trim() + "'");
            DataClass.MyMeans.getsqlcom("Delete tb_RANDP where Sut_ID='" + S_0.Text.Trim() + "'");
            DataClass.MyMeans.getsqlcom("Delete tb_WordResume where Sut_ID='" + S_0.Text.Trim() + "'");
            DataClass.MyMeans.getsqlcom("Delete tb_Individual where ID='" + S_0.Text.Trim() + "'");
            Sut_Cancel_Click(sender, e);    //调用“取消”按钮的单击事件
        }
        ...
        private void but_Table_Click(object sender, EventArgs e)
        {
            ...
            //    wordDoc.Range(ref start, ref end).Rows.HeightRule = Microsoft.Office.Interop.Word.WdRowHeightRule.wdRowHeightAtLeast;
            //    wordDoc.Range(ref start, ref end).Rows.Height = wordApp.CentimetersToPoints(float.Parse("0.8"));
            //    wordDoc.Range(ref start, ref end).Font.Size = 10;
            //    wordDoc.Range(ref start, ref end).Font.Name = "宋体";
            //    //设置表格样式
            ...
            //    //第1行显示
            //    Microsoft.Office.Interop.Word.Table.Cell(1, 5).Merge(wordDoc.Tables.Item(1).Cell(5, 6));
            //    //第1行显示
